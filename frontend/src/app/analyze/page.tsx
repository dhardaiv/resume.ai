  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950">
      ...
      <header className="sticky top-0 z-50 border-b border-slate-800/60 bg-slate-950/80 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-4">
          ...
          {appState === 'results' && (
            <button
              onClick={handleReset}
              className="ml-auto flex items-center gap-1.5 text-sm text-slate-400 hover:text-white transition-colors border border-slate-700 hover:border-slate-600 rounded-lg px-3 py-1.5"
            >
              <ResetIcon />
              New analysis
            </button>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-10 flex flex-col gap-8">
        ...
        {/* Upload + Preview section (hidden once results are in) */}
        {appState !== 'results' && (
          <>
            {/* Page heading */}
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-slate-400 text-sm">
                Upload your resume and the target job description to get an AI-powered match
                analysis.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <StepBadge
                ...
                active={canAnalyze && appState !== 'loading'}
                label="Analyze"
              />
            </div>

            {/* Upload cards + document previews */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-4">
                ...
                <DocumentPreview file={resumeFile} label="Resume" />
              </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <DocumentPreview file={jobFile} label="Job Description" />
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-2">
                  <FileBadge name={resumeFile?.name ?? ''} color="blue" />
                  <span className="text-slate-600 text-sm">↔</span>
                  <FileBadge name={jobFile?.name ?? ''} color="indigo" />
                </div>
              </div>
            </div>
      ...
      <footer className="border-t border-slate-800 py-5 text-center text-xs text-slate-600">
        resume.ai · Powered by sentence-transformers
      </footer>
    </div>
  );
}
...
function StepBadge({
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className={`text-xs transition-colors ${active || done ? 'text-slate-300' : 'text-slate-600'}`}>
        {label}
      </span>
    </div>
  );
...
function FileBadge({ name, color }: { name: string; color: 'blue' | 'indigo' }) {
  if (!name) return null;
  return (
    <span
